...
Write-Host "  |   [RADIO] SWARAM RADIO - STARTUP |" -ForegroundColor Cyan
Write-Host "  +----------------------------------+" -ForegroundColor Cyan
Write-Host ""
...
pm2 delete swaram-server 2>&1 | Out-Null
pm2 start pm2.config.js 2>&1 | Out-Null
$ErrorActionPreference = $oldEAP
Pop-Location
Start-Sleep -Seconds 2
Write-Host "        [OK] Server running on :3001" -ForegroundColor Green
...
if (-not $tunnelUrl) {
    Write-Host "        [ERROR] Could not capture tunnel URL. Check your internet connection." -ForegroundColor Red
    exit 1
}
Write-Host "        [OK] Tunnel live: $tunnelUrl" -ForegroundColor Green
...
try {
    ...
    Invoke-RestMethod -Uri "$SUPABASE_URL/rest/v1/radio_config" -Method POST -Headers $headers -Body $body | Out-Null
    Write-Host "        [OK] URL saved to Supabase" -ForegroundColor Green
} catch {
    Write-Host "        [ERROR] Supabase update failed: $_" -ForegroundColor Red
...
Write-Host ""
Write-Host "  +-----------------------------------------+" -ForegroundColor Cyan
Write-Host "  |  [OK] SWARAM IS READY FOR BROADCAST     |" -ForegroundColor Cyan
Write-Host "  +-----------------------------------------+" -ForegroundColor Cyan
